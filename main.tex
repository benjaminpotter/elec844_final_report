...
\begin{abstract}
  Air traffic control services face increasing demand while
  recruitment and training of new personnel remains slow.
  This work explores autonomous planning for air traffic control by
  ...
  expertise into automated air traffic control systems.
\end{abstract}
\acresetall
...
First, I present a novel offline method for learning the penalty
...
sampling-based planners on the air traffic control problem using real
flight data.
...
\times [-\pi, \pi)$, with $s = (x, y, z, \theta)$~\cite{}.
...
\ac{enu} conventions.
This means the $x$, $y$, and $z$ axes are aligned with east, north,
...
constraints that human air traffic controllers must consider.
...
The formalization of the planning problem for air traffic control I use is
...
\ac{mcrrt}.
The \ac{ara} implementations are inspired by Tolstaya et
al. from~\cite{tolstaya_rkk:planning_atc}.

\section{ANYTIME REPAIRING A$^*$} \label{sec:ara}

\todo{What do we do for epsilon?}
...
This discretization follows the example provided
...
I also discretize the control space $U$ and time into fixed-time
motion primitives.
These motion primitives define what states are reachable over a single edge.
Time is fixed to an interval of $\Delta t = 30$ seconds.
The altitude change is chosen from the set $u_z~\in~\{-\Delta z, 0,
\Delta z\}$ and the bearing from the set $u_\theta~\in~\{-\Delta
\theta, 0, \Delta \theta\}$.
I use the definitions from \cite{tolstaya_rkk:planning_atc} which are
$\Delta z = 6$ meters per second and $\Delta \theta = 0.025$ radians per second.
...
If the sample is accepted, I set $s_{t+1} = s'$.
Otherwise, I set $s_{t+1} = s_t$.
I iterate over candidate samples until one is accepted at which point
I proceed with the steering procedure.

I use the Metropolis-Hastings algorithm with the proposal function $g$ as a
standard Gaussian.
The proposal function takes a state $s_t$ and returns a state $s'$
according to an offset sampled from a standard Gaussian.

\begin{equation*}
  s' = g(s_t) = s_t + \mathcal{N}(0, 1)
\end{equation*}
...
A neighborhood is computed as using the \ac{knn} algorithm where $k$
is defined by Eqn.~\ref{equ:k}.
Referring to Eqn.~\ref{equ:k}, the dimensionality of the
configuration space $n$ is \numprint{4}, and the cardinality of the
search tree $|V|$ grows as the algorithm adds states.
...
The raw data for each flight is downloaded from the FlightAware API~\cite{}.
...
reference to the \ac{wgs} position of \ac{sea}~\cite{}.
...
\begin{table*}[!t]
  ...
  \caption{Experimental results from the air traffic control
  planning problem with \ac{ara} and \ac{rrt} variants.}
  \label{tab:results}
  \centering
...
penalty factor with the aim of increasing the efficiency of each expanded state.
...
\todo{explain the improvement of mcara}
...
\todo{explain how the learning method might yield a better penalty}

\todo{when goal region is expanded rrt can find solutions}

\section{CONCLUSION} \label{sec:conclusion}

In this work, I implement and compare graph-based and sampling-based
planning algorithms for air traffic control using real flight
...
First, incorporating a \ac{kdt} for efficient nearest neighbor queries
in \ac{rrt} could reduce the computational overhead that currently
limits state expansion.
Second, the penalty term could be modified to account for the
exponential effect of airplane frequency on common routes, which may
better capture the true cost of deviation.
Third, weighted sampling using the penalty factor could improve
\ac{rrt} by directly biasing samples toward low-cost regions rather
than sampling the penalty along edges.
Fourth, a systematic grid search could tune the sampling parameters to
optimize performance.
Finally, reducing the search space size or expanding the goal region
could make the problem more tractable for sampling-based methods.
Despite the limitations of the \ac{rrt} implementations tested here,
the success of \ac{ara} demonstrates that automated planning for air
traffic control is feasible when appropriate algorithmic choices are made.
The learned penalty function provides a principled way to incorporate
human expertise into automated systems, and the experimental framework
developed in this work provides a foundation for future research in
this domain.
